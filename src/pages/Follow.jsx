...
const Follow = () => {
  ...
  const handlePageClick = async (selectedPage) => {
    ...
    let data;
    if (tabActive === "unFollowed") {
      data = await getUnFollowed(selectedPage.selected + 1);
      setFollows(data);
    } else if (tabActive === "following") {
      data = await getFollowings(selectedPage.selected + 1);
      setFollows(data);
    } else if (tabActive === "follower") {
      data = await getFollowers(selectedPage.selected + 1);
      setFollows(data);
    }
    setIsLoading(false);
  };
  ...
  const handleSearch = async (event, currentPage) => {
    if (event?.key === "Enter" || !event?.key) {
      if (keywordSearch) {
        let data;
        if (tabActive === "unFollowed") {
          //   data = await searchUnFollowedAPI(currentPage, keywordSearch);
          //   setFollows(data);
