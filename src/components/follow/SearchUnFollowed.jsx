...
import { searchUnFollowedAPI } from "../../services/userService";

const SearchUnFollowed = ({ tabActive }) => {
  ...
  const getPosts = async (query) => {
    if (query) {
      setIsLoading(true);
      const data = await searchUnFollowedAPI(1, query);
      if (data?.code === 0) {
        setData(data?.data);
        setCount(data?.count);
      } else if (data?.code === 1) {
  ...
  const handlePageClick = async (selectedPage) => {
    ...
    if (query) {
      ...
      const data = await searchUnFollowedAPI(selectedPage.selected + 1, query);
      if (data?.code === 0) {
        setData(data?.data);
      }
      setIsLoading(false);
...
export default SearchUnFollowed;
