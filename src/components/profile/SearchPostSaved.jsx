...
const SearchPostSaved = ({ tabActive }) => {
  ...
  const getPosts = async (query) => {
    if (query) {
      setIsLoading(true);
      const data = await searchPostSavedAPI(1, query);
  ...
  const handlePageClick = async (selectedPage) => {
    ...
    if (query) {
      ...
      const data = await searchPostSavedAPI(selectedPage.selected + 1, query);
      if (data?.code === 0) {
        setData(data?.data);
      }
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          {data.map((item) => (
            <PostCard key={item.id} post={item} link={"/community/post"} />
          ))}
...
export default SearchPostSaved;
